import DeleteVeiculoButton from "@/components/DeleteVeiculoButton";
...
export default async function VeiculosPage() {
  ...
  const { data: veiculos, error } = await supabase.from("veiculos").select("*").order('id', { ascending: true });

  if (error) {
    return (
      <div className="min-h-screen bg-background p-8">
        <p className="text-destructive">Erro ao carregar os veículos.</p>
      </div>
    );
  }
            ...
            <table className="w-full">
              <thead className="bg-muted">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-sm font-medium">Km</th>
              ...
              <tbody className="divide-y divide-border">
                {veiculos.map((veiculo) => (
                  <tr key={veiculo.id} className="hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3 text-sm">{veiculo.quilometragem.toLocaleString()} km</td>
                    <td className="px-4 py-3 text-sm">{formatarPreco(veiculo.preco)}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/veiculos/${veiculo.id}`} className="rounded-md p-2 hover:bg-muted" title="Editar">
                          <Edit className="h-4 w-4" />
                        </Link>
                        <DeleteVeiculoButton id={veiculo.id} />
                      </div>
                    </td>
                  </tr>
                ))}
